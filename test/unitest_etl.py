...
@pytest.mark.parametrize('s3_key, s3_secret, s3_bucket, s3_dataset_path,dataset_path, dataset_dir, expected_exception',[
    ...
    ('AZUasdHWTZ2MDHCZHO6', 'IAZasdUasdHWTZ2MDHCZHO6', S3_BUCKET, S3_DATASET_PATH, DATASET_PATH, DATASET_DIR, botocore.exceptions.ClientError),
    (S3_KEY, S3_SECRET, 'BUCKET_NO_EXIST', S3_DATASET_PATH, DATASET_PATH, DATASET_DIR, botocore.exceptions.ClientError),
    (S3_KEY, S3_SECRET, S3_BUCKET, 'outputs/file_no_exist.xlsx', DATASET_PATH, DATASET_DIR, botocore.exceptions.ClientError),
    (S3_KEY, S3_SECRET, S3_BUCKET, S3_DATASET_PATH, 'folder_no_exist/file_no_exist.xlsx', DATASET_DIR, FileNotFoundError),
    ]
)
...
def test_download_dataset_from_s3(s3_key, s3_secret, s3_bucket, s3_dataset_path, dataset_path, dataset_dir, expected_exception):
    with pytest.raises(expected_exception):
        download_dataset_from_s3(s3_key, s3_secret, s3_bucket, s3_dataset_path, dataset_path, dataset_dir)
...
@pytest.mark.parametrize('dataset, expected_exception',
                            [
                                ('', FileNotFoundError),
                                ('./outputs/dataset_not_exist.xlsx', FileNotFoundError),
                            ]
                        )
...
@pytest.mark.parametrize('df_transactions, expected_exception',
                            [
                                ...
                                (pd.DataFrame({'name': ['ricardo', 'nacho'], 'last_name': ['apellido1','apellido2']}), KeyError),
                            ]
                        )
...
def  test_transform_target_customers_success():
    assert type(transform_target_customers(df_target_customers)) == pd.DataFrame
...
def  test_transform_customers_success():
    assert type(transform_customers(df_address,df_demographic)) == pd.DataFrame
